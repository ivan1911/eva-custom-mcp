...
| `EVA_USER_LOGIN` | Нет | Ваш логин в EvaTeam. Используется в `whoami` и `my_tasks`, если текущего пользователя не удаётся определить по токену. |
...
- `whoami`: пользователь, которому принадлежит API-токен.
- `my_tasks`: незакрытые задачи, где вы исполнитель, постановщик или от вас ждут ответа; можно посмотреть и по другому человеку.
