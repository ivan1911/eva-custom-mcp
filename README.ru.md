...
| `EVA_USER_LOGIN` | Для `whoami` и `my_tasks` без `person` | Логин пользователя по умолчанию в EvaTeam. Остальные инструменты работают без него; принадлежность токена не проверяется. |
...
- `task_assign`: назначить задачу на человека по логину или имени; можно сразу поставить «Ждём ответа» и сменить статус.
- `task_link_create`: связать две задачи.
- `task_time_log`: списать время.
- `task_create_from_template`: создать задачу по шаблону.

`task_create`, `task_update` и `task_assign` принимают человека как логин, уникальную часть имени или ссылку
`CmfPerson:<uuid>`. В `task_create` и `task_update` можно указать постановщика (`owner`), а в `task_create` —
спринт (`sprintCode`).

Люди:

- `person_search`: найти пользователей по части имени или логина.
- `person_get`: получить пользователя по логину, уникальной части имени или ссылке.
- `whoami`: пользователь из `EVA_USER_LOGIN`, который не обязательно является владельцем токена.
- `my_tasks`: незакрытые задачи пользователя из `EVA_USER_LOGIN` по роли исполнителя, постановщика или ожидаемого ответа. С явным `person` работает без `EVA_USER_LOGIN`; `hasMore` сообщает, что за пределами `limit` есть ещё задачи.

Релизы:

- `release_list`: список релизов, при необходимости по проекту и названию (или спринтов с `codePrefix: "SPR-"`).
- `release_tasks`: задачи релиза и точное количество по типам статусов.
- `task_add_release`: добавить релиз в задачу; уже назначенные релизы сохраняются.
